...
public final class VoiceManager: Sendable {
    ...
    func getEndpoint() async throws -> [String: String] {
    ...
    /// Create TTS request
    func createTTSRequest(request: TTSRequest) async throws -> URLRequest {
        // Validate input
    ...
    public func createVoice(
        ...
        let config = Defaults[.ttsConfig]

        let fallbackVoice = voice ?? config.defaultVoice
        let fallbackRate = rate ?? "\(config.defaultRate)"
        let fallbackPitch = pitch ?? "\(config.defaultPitch)"
        let fallbackStyle = style ?? "general"
        ...
        let results = try await withThrowingTaskGroup(of: (Int, Data).self) { group in
            for (index, segment) in segments.enumerated() {
                group.addTask {
                    ...
                    return (index, response.audioContent)
                }
            }

            // 收集返回的 (index, audio) 结果
            var results = Array<Data?>(repeating: nil, count: segments.count)
            for try await (index, audio) in group {
                results[index] = audio
            }
    ...
    // MARK: - UTILS
    /// Endpoint utilities
    final class EndpointUtils: Sendable {
        ...
        public static func getEndpoint(region: String) -> String {
            return "https://\(region).tts.speech.microsoft.com/cognitiveservices/v1"
        }
        
        /// Get the voices endpoint URL for a region
        public static func getVoicesEndpoint(region: String) -> String {
            return "https://\(region).tts.speech.microsoft.com/cognitiveservices/voices/list"
        }
        
        /// Get endpoint information
        public static func getEndpoint() async throws -> [String: String] {
        ...
        public static func getExp(from jwt: String) -> Int64 {
    ...
    final class TextUtils: Sendable {
        ...
        public static func getLocaleFromVoice(_ voice: String) -> String {
        ...
        public static func splitAndFilterEmptyLines(_ text: String) -> [String] {
        ...
        public static func mergeStringsWithLimit(_ strings: [String], minLen: Int, maxLen: Int) -> [String] {
            var result: [String] = []
            var i = 0
        ...
        public static func processMarkdownText(_ input: String) -> String {
            // 第一步：去除所有空格
    ...
    final class FileUtils:Sendable {
        ...
        func getVoiceDirectory() -> URL? {
            guard let containerURL = DatabaseManager.documentUrl() else { return nil }
       ...
       public static func FileName(text:String) throws -> URL{
            let fileName = text.sha256()
            
            guard let group = DatabaseManager.getVoiceDirectory() else {
        ...
        /// Write data to file
        public static func setCache(_ data: Data, text:String) throws -> URL {
        ...
        public static func appendToFile(_ data: Data, text: String) throws {
            let filepath  = try Self.FileName(text: text)
        ...
        public static func getCache(_ text: String) throws -> URL {
            let filepath  = try Self.FileName(text: text)
    ...
    /// SSML processor
   final class SSMLProcessor: Sendable {
        private let config: SSMLConfig
        private let patternCache: [String: NSRegularExpression]
        
       public init(config: SSMLConfig) throws {
            self.config = config
            var patternCache: [String: NSRegularExpression] = [:]
       ...
       public func escapeSSML(_ ssml: String) -> String {
    ...
    struct MicrosoftVoice: Identifiable,Codable , Sendable{
    ...
    /// SSML request model
    struct SSMLRequest: Codable,Sendable {
        let ssml: String
        let voice: String
        let rate: String
        let pitch: String
    ...
    struct TagPattern:Codable,Sendable {
        let name: String
        let pattern: String
    }

    /// SSML configuration
    struct SSMLConfig:Codable, Sendable {
